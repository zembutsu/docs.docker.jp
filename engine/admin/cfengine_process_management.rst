...
CFEngine は ``ENTRYPOINT`` で指定したコマンドの ``ベースネーム`` （最終的に実行するコマンドの名前）にあたるプロセス・テーブルをスキャンし、 ``ベースネーム`` のプロセスが見つからなければ、それを開始しようとします。例えば、コンテナを ``docker run "/path/to/my/application パラメータ"`` で開始したら、CFEngine は ``application`` という名前のプロセスを探し、コマンドを実行します。もし ``application`` にあたるエントリがプロセス・テーブルになければ、 CFEngine は ``/path/to/my/application parameters`` でアプリケーションを再度起動しようとします。このプロセス・テーブルの確認は、毎分実行されます。
