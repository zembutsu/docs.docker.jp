.. -*- coding: utf-8 -*-
.. URL: https://docs.docker.com/engine/reference/commandline/container_commit/
.. SOURCE: 
   doc version: 20.10
      https://github.com/docker/docker.github.io/blob/master/engine/reference/commandline/container_commit.md
      https://github.com/docker/docker.github.io/blob/master/_data/engine-cli/docker_container_commit.yaml
...
.. list-table::
   ...
   * - ``--author``, ``-a``
     - 
     - :ruby:`作者 <author>` （例： "John Hannibal Smith <hannibal@a-team.com>" ）
   * - ``--change``, ``-c``
     - 
     - 作成するイメージに対し、Dockerfile 命令を適用
   * - ``--message``, ``-m``
     - 
     - コミットのメッセージ
   * - ``--pause``, ``p``
     - ``true``
     - コミット中はコンテナを :ruby:`一時停止 <pause>`
